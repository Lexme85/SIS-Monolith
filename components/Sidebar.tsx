...
import { Euro, Calculator, Database, ClipboardList, ShieldAlert, UserCheck, Zap, BrainCircuit, Library } from 'lucide-react';
        ...
        clientData.diagnoses.forEach(diag => {
            ...
            if (logicEntry && logicEntry.specificItems) {
                const matchingItems = logicEntry.specificItems.filter(item => 
                    item.tf === prefix && selectedSymptoms.includes(item.n)
                );
  ...
  return (
    <div className={`w-[280px] bg-slate-900 flex flex-col shrink-0 overflow-y-auto h-full border-r border-slate-800 ${className}`}>
      <div className="py-8">
        ...
        <NavItem view="psychAssessment" label="Psych. Assessment" icon={BrainCircuit} />
        <NavItem view="diagDetails" label="Diagnose-Details" icon={Database} />
        <NavItem view="archive" label="Klienten-Archiv" icon={Library} />
      </div>
